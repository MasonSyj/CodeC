...
void listfunc(void){
   ...
   if (opcode == CAR){
      ...
      if (notpass){
         newlisps->arr[newlisps->top++] = list2lisp();
      }
      #endif
      token->currentrow++;
      ...
      if (notpass){
         newlisps->arr[newlisps->top++] = list2lisp();
         newlisps->arr[newlisps->top++] = lisp_cons(newlisps->arr[newlisps->top - 2], newlisps->arr[newlisps->top - 1]);
         newlisps->top -= 2;
...
void intfunc(void){
   ...
   if (opcode == PLUS){
      ...
      int value2;
      if (notpass){
         assert(lisp_isatomic(newlisps->arr[newlisps->top]));
         value1 = lisp_getval(newlisps->arr[newlisps->top]);
      }
      #endif
      islist();
      #ifdef INTERP
      if (notpass){
         newlisps->arr[newlisps->top] = list2lisp();
         assert(lisp_isatomic(newlisps->arr[newlisps->top]));
         value2 = lisp_getval(newlisps->arr[newlisps->top]);
      ...
      #ifdef INTERP
      if (notpass){
         newlisps->arr[newlisps->top++] = lisp_atom(lisp_length(newlisps->arr[newlisps->top]));
      }
      #endif
   }
}
...
bool isvar(void){
   return (int)strlen(token->word[token->currentrow]) == 1 
   && isupper(token->word[token->currentrow][0]);
}
...
bool isliteral(void){
   ...
   return token->word[token->currentrow][0] == '\'' 
   && token->word[token->currentrow][lastchar] == '\'';
}
